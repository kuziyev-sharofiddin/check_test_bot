...
async def begin(message: types.Message):
        data["step"] = steps["enter_name_id"]
        await message.answer("Iltimos loginingizni kiriting")
        
@dp.message_handler(lambda message: data["step"] == steps["enter_name_id"])
async def begin(message: types.Message):
    data["step"] = steps["enter_name_id"]
...
async def group_view(message: types.Message):
    data["step"] = steps["choose_group_students"]
    await message.answer("Quyidagilardan birini tanlang", 
                        reply_markup=group_buttons(data["state"]["teacher"][0]['id']))
    
@dp.message_handler(lambda message: data["step"] == steps["choose_group_students"])
...
@dp.message_handler(lambda message: data["step"] == steps["test_name_enter"])
